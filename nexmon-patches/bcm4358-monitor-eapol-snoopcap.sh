...
glob = (
    ...
    "static volatile unsigned short g_lbuf_len  = 1;\n"
    "static volatile unsigned short g_rxhdr_len = 1;\n"
    ...
    "    unsigned int   i, blen, j;\n"
    "    if (len < 38 || len > 1600)\n"
    "        return;\n"
    "    // The packet here carries an rx prefix + 802.11 + LLC/SNAP + EAPOL.\n"
    "    // Scan for the LLC/SNAP EAPOL signature (aa aa 03 .. 88 8e) and stash\n"
    "    // the 802.1X body that follows -- this is the EAPOL at this RX stage,\n"
    "    // full or not; its length tells us whether the cut is upstream.\n"
    "    for (i = 0; i + 8 <= len && i < 240; i++) {\n"
    "        if (data[i] == 0xAA && data[i+1] == 0xAA && data[i+2] == 0x03 &&\n"
    "            data[i+6] == 0x88 && data[i+7] == 0x8E) {\n"
    "            // measurement: the d11 rxhdr framelen sits at data[0x0E] (the\n"
    "            // rx prefix starts at data[0]); compare with the lbuf len to see\n"
    "            // whether the ucode wrote a short framelen or the DMA capped it.\n"
    "            g_lbuf_len  = (unsigned short)len;\n"
    "            g_rxhdr_len = (unsigned short)(data[0x0E] | (data[0x0F] << 8));\n"
    "            blen = len - (i + 8);\n"
    "            if (blen > sizeof(g_eapol_data)) blen = sizeof(g_eapol_data);\n"
    "            for (j = 0; j < blen; j++)\n"
    "                g_eapol_data[j] = data[i + 8 + j];\n"
    "            g_eapol_len = (unsigned short)blen;   // 802.1X body length here\n"
    "            return;\n"
    "        }\n"
    "    }\n"
    "}\n"
...
splice = anchor + (
    ...
    "                    unsigned char *mbody = llc + 8;     // 802.1X body in 802.11\n"
    "                    unsigned char *dbg = mbody + 81;    // Key-MIC area (zero)\n"
    "                    unsigned int saved;\n"
    "                    // MEASUREMENT: stamp the RX-stage lengths so the pcap tells\n"
    "                    // us where the cut is. rxhdr_len full + lbuf_len short =>\n"
    "                    // DMA cap; rxhdr_len short => ucode wrote a short framelen.\n"
    "                    dbg[0]  = 0xDE; dbg[1]  = 0xAD;\n"
    "                    dbg[2]  = g_eapol_len;  dbg[3]  = g_eapol_len >> 8;\n"
    "                    dbg[4]  = g_lbuf_len;   dbg[5]  = g_lbuf_len >> 8;\n"
    "                    dbg[6]  = g_rxhdr_len;  dbg[7]  = g_rxhdr_len >> 8;\n"
    "                    dbg[8]  = g_rx_caller;  dbg[9]  = g_rx_caller >> 8;\n"
    "                    dbg[10] = g_rx_caller >> 16; dbg[11] = g_rx_caller >> 24;\n"
    "                    saved = p->len;\n"
    "                    p->len = hdrlen + 8 + 81 + 16 + 6;\n"
